...
public class task1Utils {
    ...
    //Method created to find the exact team with the max number of Devs, it returns the string team_Code
    public static String findTeamWithMaxDevs(ArrayList<Team> team_List) {
        ...
        return getCounter(teamDevsCount);
    }

    //Method to find the first team that has worked for the entirety of its developing time
    public static String findTeamWithAllDT(GameTeamAssociations teamsToGames,
        ...
        //Finding the maximum count of Devs among all Teams
        return getCounter(teamDTCount);
    }

    //This method returns the common skill among the teams in our team_List
    ...
    //This method has been created to prevent duplications , two methods are using it
    private static String getCounter(Map<String, Integer> mapCount){
        //Max Counter for our sorting
        int maxCount = Collections.max(mapCount.values());

        //Creating a list to store team codes
        List<String> dataString = new ArrayList<>();

        //Adding the teams with the values to the map
        for (Map.Entry<String, Integer> entry : mapCount.entrySet()) {
            if (entry.getValue() == maxCount) {
                dataString.add(entry.getKey());
            }
        }

        //Sorting the list of teams , using collections makes it simpler
        Collections.sort(dataString);

        //Check if the list is not empty
        if (!dataString.isEmpty()) {
            return dataString.get(0); //Returning the first team code in the sorted list
        } else {
